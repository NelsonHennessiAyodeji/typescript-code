...
// Type Aliases: Representing our types with different names
...
// Defining a math function type for two number parameters that returns a number, that we can use later
...
const sumAllII = (a: number = 2, b: number, c: number = 2): number => {
    return a + b + c;
}
...
const infinite = () => {
    ...
    while (true) {
        i++
        if (i > 100) break // This solves the endless loop
    }
}
